-- Stronger verify guards + clearer outside-geofence messages (include saved vs phone coords)
...
begin
  ...
  -- Prevent accidental verify at Null Island when UI sent Number('') = 0
  ...
  if not public.has_management_role() and not public.is_assigned_to_project(p_project_id) then
    ...
    if v_distance is null or v_distance > v_radius then
      ...
      v_reason :=
        ...
        ' m). Phone GPS: ' || round(p_latitude::numeric, 5)::text || ', ' || round(p_longitude::numeric, 5)::text ||
        '. Project GPS: ' || round(v_project.latitude::numeric, 5)::text || ', ' || round(v_project.longitude::numeric, 5)::text ||
        '. Re-verify the project with “Use my current GPS” while standing at the site, then try again.';
    else
      v_result := 'approved';
      v_reason := null;
    end if;
